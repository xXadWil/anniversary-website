...
export default function Page() {
  ...
  function unlock() {
    ...
    sessionStorage.setItem('anniversary-unlocked', 'true')
  ...
  function readFile(file: File, callback: (value: string) => void) {
  ...
  return (
    <main className="anniversary-app">
      {!unlocked && !transitioning && (
        <section
          className="lock-screen"
          style={background ? { backgroundImage: `linear-gradient(135deg, rgba(40,18,28,.68), rgba(115,49,66,.55)), url(${background})` } : undefined}
        >
          <div className="lock-glow" />
          <div className={`lock-card ${error ? 'shake' : ''}`}>
            <div className="lock-mark"><Heart size={20} fill="currentColor" /></div>
            <p className="eyebrow">A little place for us</p>
            <h1>Our story,<br /><em>kept softly.</em></h1>
            <p className="lock-copy">This space is just for you. Enter our special date to open it.</p>
            <div className="code-row">
              <input
                ...
                placeholder="••••••"
              />
              <button type="button" onClick={unlock} aria-label="Unlock memory"><LockKeyhole size={18} /></button>
            </div>
            <p className={`error-copy ${error ? 'visible' : ''}`}>That date needs another try.</p>
            <span className="lock-hint">six digits · one beautiful beginning</span>
          </div>
      ...
      {transitioning && (
        <div className="flower-flood" aria-label="Opening our story">
      ...
      {unlocked && (
        <div className="dashboard">
          <header className="topbar">
            <div className="brand"><span className="brand-dot"><Heart size={13} fill="currentColor" /></span> our little archive</div>
            <div className="topbar-date">august 31, 2024 <span>·</span> still here</div>
          </header>
          ...
          <section className="hero">
            ...
            <div className="counter">
              ...
              <div><strong>{String(time.hours).padStart(2, '0')}</strong><span>hours</span></div><i>:</i>
              <div><strong>{String(time.minutes).padStart(2, '0')}</strong><span>minutes</span></div><i>:</i>
              <div><strong>{String(time.seconds).padStart(2, '0')}</strong><span>seconds</span></div>
            </div>
            <a className="scroll-cue" href="#portals"><ChevronDown size={17} /> scroll down to explore</a>
          </section>
          ...
          <footer>
            <Heart size={13} fill="currentColor" /> made with all my love <Heart size={13} fill="currentColor" />
          </footer>
        </div>
      )}
        ...
        <ModalShell onClose={() => setModal(null)}>
          ...
          {modal === 'notes' && (
            <>
              ...
              <p className="eyebrow rose">a growing list</p>
...
function Portal({ image, title, subtitle, className, onClick }: { image: string; title: string; subtitle: string; className: string; onClick: () => void }) {
  return (
    <button type="button" className={`portal ${className}`} onClick={onClick}>
      <div className="custom-portal-art">
        <img 
          src={image} 
          alt="" 
          style={{ 
      ...
      <div className="portal-meta">
        <span className="portal-icon"><Heart size={15} fill="currentColor" /></span>
        <span style={{ minWidth: 0, overflow: 'hidden' }}>
          <b style={{ display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</b>
          <small style={{ display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{subtitle}</small>
        </span>
        <span className="portal-action">→</span>
      </div>
    </button>
  )
}
